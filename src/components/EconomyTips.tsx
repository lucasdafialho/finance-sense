    ...
    <Card className="rounded-3xl shadow-md border-none overflow-hidden mb-6">
      <CardHeader className="p-6 pb-4">
        <div className="flex items-center mb-2">
          ...
          <CardTitle className="text-lg font-semibold">Dicas de Economia</CardTitle>
        </div>
        <CardDescription>
        ...
        <div className="min-h-[200px] flex flex-col justify-between">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              exit={{ opacity: 0, x: -20 }}
              className="bg-finance-light/50 p-5 rounded-2xl mb-4"
            >
              <h4 className="font-medium text-finance-accent mb-2">{currentTip.title}</h4>
              <p className="text-sm text-finance-text mb-3">{currentTip.description}</p>
              ...
              {economyTips.map((_, index) => (
                <span 
                  ...
                  className={`block h-1.5 w-6 rounded-full ${
                    index === currentTipIndex ? 'bg-finance-primary' : 'bg-muted'
                  }`}
                />
              ))}
            </div>
